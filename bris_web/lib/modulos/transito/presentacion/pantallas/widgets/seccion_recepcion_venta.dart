...
class _SeccionRecepcionVentaState extends State<SeccionRecepcionVenta> {
  void _mostrarDialogoVenta([VentaWebModelo? ventaExistente]) {
    ...
    final precioCtrl = TextEditingController(text: ventaExistente?.precioUnitario.toString() ?? '');
    final otraPlantaCtrl = TextEditingController(
      text: (ventaExistente != null && plantaSeleccionada == 'OTROS') ? ventaExistente.lugar : ''
    );
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (contextDialog, setStateDialog) => AlertDialog(
          ...
          title: Text(
            ...
            style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF15181A), fontSize: 20),
          ),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0D5C75), foregroundColor: Colors.white),
        ...
        children: [
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.storefront_outlined, color: Color(0xFF1B5E20), size: 20),
                    ),
                    const SizedBox(width: 12),
                    Text('Recepción en Planta', style: GoogleFonts.sora(fontSize: 18, fontWeight: FontWeight.bold, color: const Color(0xFF15181A))),
                  ],
                ),
                ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0D5C75), foregroundColor: Colors.white, elevation: 0),
                ),
              ],
            ),
          ),
          if (widget.ventas.isEmpty)
            const Padding(
              ...
              child: Center(child: Text('No hay recepciones registradas.', style: TextStyle(color: Color(0xFF94A3B8)))),
            )
          else
            ListView.separated(
              shrinkWrap: true,
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.factory_outlined, color: Color(0xFF64748B), size: 20),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('${v.lugar} - ${v.producto}', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF15181A), fontSize: 15)),
                            const SizedBox(height: 4),
                            Text('${v.kilos.toStringAsFixed(2)} kg @ S/ ${v.precioUnitario.toStringAsFixed(2)}', style: const TextStyle(color: Color(0xFF64748B), fontSize: 13)),
                          ],
                        ),
                      ),
                      Text('S/ ${v.total.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF0D5C75), fontSize: 16)),
                      const SizedBox(width: 16),
                      Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          IconButton(icon: const Icon(Icons.edit_outlined, color: Color(0xFF64748B), size: 20), onPressed: () => _mostrarDialogoVenta(v)),
                          IconButton(icon: const Icon(Icons.delete_outline, color: Colors.redAccent, size: 20), onPressed: () => widget.onEliminar(v.id)),
                        ],
                      ),
                    ],
                  ),
